...
import type { BskyFacet, BlobLike } from "../_migration-helpers";
...
const reportedPartialActivityErrors = new Set<string>();

function reportPartialActivityErrors(
  ...
  const dedupeKey = `${operationName}:${messages.join("|")}`;

  if (reportedPartialActivityErrors.has(dedupeKey)) {
    return;
  }

  reportedPartialActivityErrors.add(dedupeKey);
  console.warn(
    `[graphql/indexer/activities] ${operationName} returned partial data: ${messages.join("; ")}`,
  );
}

function hasPartialResponseData<ResponseType>(response: {
  data?: unknown;
}): response is { data: ResponseType } {
  return response.data !== undefined;
}

type GraphQLExecutionResult<ResponseType> = {
  data?: ResponseType;
...
function isGraphQLExecutionResult<ResponseType>(
  value: unknown,
): value is GraphQLExecutionResult<ResponseType> {
  return typeof value === "object" && value !== null;
}

async function requestActivityDocument<ResponseType, VariablesType extends object>(
  operationName: string,
  document: TadaDocumentNode<ResponseType, VariablesType>,
  ...
  try {
    ...
    throw new Error(
      `[graphql/indexer/activities] ${operationName} returned a non-JSON response (${response.status}).`,
    );
  }

  if (!isGraphQLExecutionResult<ResponseType>(parsedBody)) {
    throw new Error(
      `[graphql/indexer/activities] ${operationName} returned an invalid GraphQL payload (${response.status}).`,
    );
  }

  reportPartialActivityErrors(operationName, parsedBody.errors);

  if (hasPartialResponseData<ResponseType>(parsedBody)) {
    return parsedBody.data;
  }

  throw new Error(
    `[graphql/indexer/activities] ${operationName} returned no data (${response.status}).`,
  );
}
...
type ActivityResponse = ResultOf<typeof byDidDocument>;
type ActivityListPageResponse = ResultOf<typeof activityListPageDocument>;
type ActivityByUriResponse = ResultOf<typeof activityByUriDocument>;
type FundingConfigResponse = ResultOf<typeof fundingConfigDocument>;
type ActivityDetailNode = ConnectionNode<ActivityResponse["orgHypercertsClaimActivity"]>;
type ActivityByUriNode = NonNullable<ActivityByUriResponse["orgHypercertsClaimActivityByUri"]>;
type ActivityNode = ActivityDetailNode | ActivityByUriNode;
type ActivityListPageNode = ConnectionNode<ActivityListPageResponse["orgHypercertsClaimActivity"]>;
type ActivityImageNode = ActivityNode["image"];
type FundingConfigNode = NonNullable<
  FundingConfigResponse["appGainforestFundingConfigByUri"]
>;
...
async function fetchFundingConfig(did: string, rkey: string): Promise<Activity["fundingConfig"]> {
  const res = await requestActivityDocument("ActivityFundingConfig", fundingConfigDocument, {
    uri: buildFundingConfigUri(did, rkey),
  });

  const fc: FundingConfigNode | null = res.appGainforestFundingConfigByUri;
  if (!fc) return null;
...
async function normalizeActivities(nodes: ActivityNode[], orgMap: Map<string, ActivityOrgInfo>): Promise<Activity[]> {
  return Promise.all(
...
function sortActivityNodesDescending(nodes: ActivityNode[]): ActivityNode[] {
...
async function fetchTierMatchedActivityNodes(tier: string): Promise<ActivityNode[]> {
  ...
  const detailNodes = await Promise.all(
    candidateUris.map(async (uri) => {
      try {
        const response = await requestActivityDocument("ActivityByUri", activityByUriDocument, {
          uri,
        });
...
export function fetch(params: ByDidAndOrgParams): Promise<Result<ByDidAndOrgParams>>;
export function fetch(params: ByDidParams): Promise<Result<ByDidParams>>;
export function fetch(params: ListParams): Promise<Result<ListParams>>;
export function fetch(params: Params): Promise<Result<Params>> {
  return fetchAny(params);
}

export async function fetchAny(params: Params): Promise<Result<Params>> {
  if ("orgDid" in params) {
    const res = await requestActivityDocument("ActivitiesByDid", byDidDocument, {
      did: params.did,
      first: 100,
      after: undefined,
    });
    ...
    return { activities, org: orgMap.get(params.orgDid) ?? null };
  }
  ...
  if ("did" in params) {
    ...
    return await normalizeActivities(nodes, orgMap);
  }
  ...
  while (filteredNodes.length < first) {
    const res = await requestActivityDocument("ActivitiesListPage", activityListPageDocument, {
      first,
...
export async function fetchCidByAtUri(activityUri: string): Promise<string | null> {
  ...
  const res = await requestActivityDocument("ActivityByUri", activityByUriDocument, {
    uri: activityUri,
  });
