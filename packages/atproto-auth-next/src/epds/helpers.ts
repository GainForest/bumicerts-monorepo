 ...
 * Based on the ePDS integration guide:
 * https://github.com/hypercerts-org/ePDS/blob/main/docs/tutorial.md
 *
 * This file is server-only (uses node:crypto).
 */
 ...
 * Required for ES256 JWTs. Internal use only.
 */
 ...
 *
 * ePDS always rejects the first request with 400 + `dpop-nonce` header.
...
export async function fetchWithDpopRetry(
  ...
  // ePDS rejects first request with 400/401 + dpop-nonce — retry with nonce
  if (!res.ok) {
