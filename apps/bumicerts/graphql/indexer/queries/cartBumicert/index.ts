...
type FundingConfigNode = NonNullable<
  ResultOf<typeof fundingConfigDocument>["appGainforestFundingConfigByUri"]
>;
...
function normalizeFundingStatus(
  status: FundingConfigNode["status"],
): FundingConfigData["status"] | null {
  switch (status) {
    case "coming-soon":
    case "open":
    case "paused":
...
export async function fetch(params: Params): Promise<CartBumicertItem | null> {
  ...
  const activityRes = await graphqlClient.request(activityDocument, {
  ...
  const [profileRes, fundingConfigRes] = await Promise.allSettled([
    graphqlClient.request(actorProfileDocument, {
      did: activity.did,
    }),
    graphqlClient.request(fundingConfigDocument, {
  ...
  const profile =
    profileRes.status === "fulfilled"
      ...
      : null;
  const rawFc: FundingConfigNode | null =
  ...
  return {
    ...
    fundingConfig: rawFc
      ? {
          ...
          status: normalizeFundingStatus(rawFc.status),
        }
      : null,
  } satisfies CartBumicertItem;
}
