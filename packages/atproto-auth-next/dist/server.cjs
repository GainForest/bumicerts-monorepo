...
__export(server_exports, {
  ...
  createAuthorizeHandler: () => createAuthorizeHandler,
  createCallbackHandler: () => createCallbackHandler,
  createClientMetadataHandler: () => createClientMetadataHandler,
  createEpdsCallbackHandler: () => createEpdsCallbackHandler,
  createEpdsLoginHandler: () => createEpdsLoginHandler,
  createJwksHandler: () => createJwksHandler,
  createLogoutHandler: () => createLogoutHandler,
  createOAuthSetup: () => createOAuthSetup,
  fetchProfile: () => fetchProfile,
  getAuthenticatedAgent: () => getAuthenticatedAgent,
...
// src/oauth-client.ts
var import_oauth_client_node = require("@atproto/oauth-client-node");
...
function createOAuthClient({
  ...
  const key = new import_jwk_jose.JoseKey(rawJwk);
  if (loopback) {
...
function createAuthorizeHandler(client, options = {}) {
  return async function POST(req) {
    ...
    return import_server2.NextResponse.json({ url: authUrl.toString() });
  };
}
function createCallbackHandler(client, sessionConfig, options = {}) {
  return async function GET(req) {
    ...
    if (success) {
      (0, import_navigation.redirect)(options.redirectTo ?? "/");
    } else {
      (0, import_navigation.redirect)("/?error=auth_failed");
    }
  };
}
function createLogoutHandler(client, sessionConfig, options = {}) {
  return async function POST() {
    ...
    if (options.redirectTo) {
      return import_server2.NextResponse.redirect(options.redirectTo);
    }
